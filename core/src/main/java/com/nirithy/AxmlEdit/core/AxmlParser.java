...
import static com.nirithy.AxmlEdit.core.NodeVisitor.TYPE_INT_BOOLEAN;
import static com.nirithy.AxmlEdit.core.NodeVisitor.TYPE_STRING;
...
public class AxmlParser implements AxmlConstants {
    ...
    public Object getAttrValue(int i) {
        ...
        if (i == idAttribute) {
            return TypedValue.wrapId(v, getAttrRawString(i));
        } else if (i == styleAttribute) {
            return TypedValue.wrapStyle(v, getAttrRawString(i));
        } else if (i == classAttribute) {
            return TypedValue.wrapClass(v, getAttrRawString(i));
        }
        ...
        for (int p = in.position(); p < fileSize; p = in.position()) {
            ...
            switch (type) {
                ...
                case RES_STRING_POOL_TYPE:
                    strings = AxmlStringPool.read(in);
                    in.position(p + size);
                    continue;
