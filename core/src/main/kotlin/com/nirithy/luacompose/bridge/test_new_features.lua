...
local function main()
    ...
    local SWIPE_THRESHOLD = 150  -- 滑动阈值（像素）
    ...
    return compose.Column {
        ...
        children = {
                ...
                children = {
                    ...
                    compose.Text {
                        ...
                        modifier = compose.Modifier()
                            ...
                            .graphicsLayerLambda(function()
                                local a = sat1.value
                                return {scaleX = a, scaleY = a, alpha = a, rotationZ = 0}
                            end)
                            .offsetLambda(function()
                                local a = sat1.value
            ...
            compose.Box {
                ...
                contentAlignment = "Center",
                children = {
                    -- 底层卡片 2（最深）— 颜色/缩放/偏移随滑动进度渐变
                    compose.Card {
                        color = 0x00000000,  -- 透明，由 drawBehind 绘制渐变背景
                        modifier = compose.Modifier()
                            .fillMaxWidth()
                            .paddingHorizontal(20)
                            .height(160)
                            .offsetLambda(function()
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                return {x = 0, y = 20 - 10 * p}
                            end)
                            .graphicsLayerLambda(function()
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                return {scaleX = 0.88 + 0.06 * p, scaleY = 0.88 + 0.06 * p, alpha = 1.0, rotationZ = 0}
                            end)
                            .drawBehind(function(draw)
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                local c1 = swipeColors[swipeTopIdx.value + 3] or 0xFFBDBDBD
                                local c2 = swipeColors[swipeTopIdx.value + 2] or 0xFFBDBDBD
                                draw.drawRect(0, 0, draw.getWidth(), draw.getHeight(), lerpColor(c1, c2, p))
                            end),
                        elevation = 0,
                        children = {
                            compose.Box {
                                ...
                                children = {
                                    compose.Text {
                                        text = "卡片 " .. tostring(swipeTopIdx.value + 2),
                    ...
                    compose.Card {
                        ...
                        modifier = compose.Modifier()
                            ...
                            .offsetLambda(function()
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                return {x = 0, y = 10 - 10 * p}
                            end)
                            .graphicsLayerLambda(function()
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                return {scaleX = 0.94 + 0.06 * p, scaleY = 0.94 + 0.06 * p, alpha = 1.0, rotationZ = 0}
                            end)
                            .drawBehind(function(draw)
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                local c1 = swipeColors[swipeTopIdx.value + 2] or 0xFFBDBDBD
                                local c2 = swipeColors[swipeTopIdx.value + 1] or 0xFFBDBDBD
                                draw.drawRect(0, 0, draw.getWidth(), draw.getHeight(), lerpColor(c1, c2, p))
                            end),
                        elevation = 1,
                        children = {
                            compose.Box {
                                ...
                                children = {
                                    compose.Text {
                                        text = "卡片 " .. tostring(swipeTopIdx.value + 1),
                                        fontSize = 24, fontWeight = "Bold",
                                        color = 0xFFFFFFFF,
                                    },
                                },
                    ...
                    -- 顶层卡片（可滑动）— 无旋转，速度减半，透明度随滑动降低
                    compose.Card {
                        color = swipeColors[swipeTopIdx.value + 1] or swipeColors[1],
                        modifier = compose.Modifier()
                            .fillMaxWidth()
                            .paddingHorizontal(20)
                            .height(160)
                            ...
                            .graphicsLayerLambda(function()
                                local p = math.min(1.0, math.abs(swipeDragX.value) / SWIPE_THRESHOLD)
                                return {scaleX = 1.0, scaleY = 1.0, alpha = 1.0 - p * 0.3, rotationZ = 0}
                            end)
                            .pointerInputFull(
                                function() end,
                                function(dx, dy)
                                    -- ★ 速度减半，避免一次手势飞出阈值
                                    swipeDragX.value = swipeDragX.value + dx * 0.5
                                end,
                                function()
                                    if math.abs(swipeDragX.value) > SWIPE_THRESHOLD then
                        ...
                        children = {
                            compose.Box {
                                ...
                                children = {
                                    compose.Text {
                                        text = "卡片 " .. tostring(swipeTopIdx.value + 1) .. " ← 滑动 →",
                                        fontSize = 20,
                                        fontWeight = "Bold",
                                        color = 0xFFFFFFFF,
                                    },
                            ...
                            compose.Box {
                                ...
                                children = {
                                    compose.Text {
                                        ...
                                        color = compose.Theme.onPrimary,
                                        modifier = compose.Modifier().rotateLambda(function()
                                            return fabRotation.value
                                        end),
                                    },
                                },
                            },
