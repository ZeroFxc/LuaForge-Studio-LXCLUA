...
object LuaStringUtil {
    fun find(s: String, pat: String): MatchResult? {
        return find(s, pat, 1, false)
    }

    fun find(s: String, pat: String, init: Int): MatchResult? {
        return find(s, pat, init, false)
    }

    fun find(s: String, pat: String, init: Int, fastMatch: Boolean): MatchResult? {
        return str_find_aux(s, pat, init, fastMatch, true)
    }

    fun gmatch(src: String, pat: String): GMatchAux {
        return GMatchAux(src, pat, false)
    }

    fun gfind(src: String, pat: String): GMatchAux {
        return GMatchAux(src, pat, true)
    }

    @JvmOverloads
    fun sub(s: String, start: Int, end: Int = -1): String {
        var start = start
        var end = end
        val l = s.length
    ...
    fun gsub(src: String, p: String, repl: Any): MatchResult? {
        try {
    ...
    fun gsub(src: String, p: String, repl: Any, max_s: Int): MatchResult {
        val srclen = src.length
        var lastmatch = -1 /* end of last match */
        val anchor = p.length > 0 && p.get(0) == '^'
        val lbuf = Buffer(srclen)
        val ms = MatchState(src, p)
        ...
        while (n < max_s) {
            ...
            if (res != -1 && res != lastmatch) {  /* match? */
                ...
                lbuf.append(src.get(soffset++))
            else break /* end of subject */
            if (anchor) break
        }
        lbuf.append(src.substring(soffset, srclen))
        return MatchResult(lbuf.tostring(), n)
    }

    fun match(s: String, pat: String): MatchResult? {
        return match(s, pat, 1, false)
    }

    fun match(s: String, pat: String, init: Int): MatchResult? {
        return match(s, pat, init, false)
    }

    fun match(s: String, pat: String, init: Int, fastMatch: Boolean): MatchResult? {
        return str_find_aux(s, pat, init, fastMatch, false)
    }

    fun rep(s: String, n: Int): String {
        val buf = StringBuilder(s.length * n)
        for (i in 0..<n) {
            buf.append(s)
        }
        return buf.toString()
    ...
    private fun str_find_aux(
        ...
        if (fastMatch) {
            ...
            if (result != -1) {
                return MatchResult(result + 1, result + pat.length)
            }
        } else {
            val ms = MatchState(s, pat)
            var anchor = false
            var poff = 0
            if (pat.length > 0 && pat.get(0) == '^') {
                anchor = true
                poff = 1
            }
            ...
            do {
                ...
                if ((ms.match(soff, poff).also { res = it }) != -1) {
                    if (find) {
                        return LuaStringUtil.MatchResult(
                            soff + 1,
                            res,
                            ms.push_captures(false, soff, res)!!
                        )
                    } else {
                        return LuaStringUtil.MatchResult(ms.push_captures(true, soff, res)!!)
                    }
                }
    ...
    private fun indexOfAny(pat: String, specials: CharArray): Int {
        for (c in specials) {
            if (pat.indexOf(c) > -1) return 1
        }
        return -1
    }
    ...
    private val L_ESC = '%'.code
    private val SPECIALS = "^$*+?.([%-".toCharArray()
    ...
    private val CHAR_TABLE: ByteArray
    private val EMOJI_TABLE = intArrayOf(
        8211,
        8212,
        8224,
        8225,
    ...
    init {
        ...
        for (i in 0..127) {
            ...
            if ((c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F') || (c >= '0' && c <= '9')) {
                CHAR_TABLE[i] = (CHAR_TABLE[i].toInt() or MASK_HEXDIGIT.toInt()).toByte()
            }
            if ((c >= '!' && c <= '/') || (c >= ':' && c <= '@') || (c >= '[' && c <= '`') || (c >= '{' && c <= '~')) {
                CHAR_TABLE[i] = (CHAR_TABLE[i].toInt() or MASK_PUNCT.toInt()).toByte()
            }
            if ((CHAR_TABLE[i].toInt() and (MASK_LOWERCASE.toInt() or MASK_UPPERCASE.toInt())) != 0) {
                CHAR_TABLE[i] = (CHAR_TABLE[i].toInt() or MASK_ALPHA.toInt()).toByte()
            }
        }

        CHAR_TABLE[' '.code] = MASK_SPACE
        CHAR_TABLE['\r'.code] = (CHAR_TABLE['\r'.code].toInt() or MASK_SPACE.toInt()).toByte()
        CHAR_TABLE['\n'.code] = (CHAR_TABLE['\n'.code].toInt() or MASK_SPACE.toInt()).toByte()
        CHAR_TABLE['\t'.code] = (CHAR_TABLE['\t'.code].toInt() or MASK_SPACE.toInt()).toByte()
        CHAR_TABLE[0x0B /* '\v' */] = (CHAR_TABLE[0x0B].toInt() or MASK_SPACE.toInt()).toByte()
        CHAR_TABLE[0x0C /* '\f' */] = (CHAR_TABLE[0x0C].toInt() or MASK_SPACE.toInt()).toByte()
    }

    class GMatchAux(src: String, pat: String, find: Boolean) {
        private val srclen: Int
        private val ms: MatchState
        private val find: Boolean
        private var soffset: Int
        private var lastmatch: Int

        init {
            this.srclen = src.length
            this.ms = MatchState(src, pat)
            this.soffset = 0
            this.lastmatch = -1
            this.find = find
        }

        fun invoke(): MatchResult? {
            while (soffset <= srclen) {
                ...
                if (res >= 0 && res != lastmatch) {
                    ...
                    soffset = res
                    lastmatch = soffset
                    if (find) return LuaStringUtil.MatchResult(
                        soff + 1,
                        res,
                        ms.push_captures(false, soff, res)!!
                    )
                    else return LuaStringUtil.MatchResult(ms.push_captures(true, soff, res)!!)
                }
                soffset++
            }
            return null
    ...
    class MatchState internal constructor(val s: String, val p: String) {
        var c: CharArray
        ...
        init {
            ...
            this.cinit = IntArray(MAX_CAPTURES)
            this.clen = IntArray(MAX_CAPTURES)
        ...
        private fun add_s(lbuf: Buffer, ns: String, soff: Int, e: Int) {
            val news = ns
            val l = news.length
            var i = 0
            while (i < l) {
                ...
                if (b.code != L_ESC) {
                    ...
                    ++i // skip ESC
                    b = (if (i < l) news.get(i) else 0.toChar())
                    if (!Character.isDigit(b)) {
                        if (b.code != L_ESC) error(
                            "invalid use of '" + L_ESC.toChar() +
                                    "' in replacement string: after '" + L_ESC.toChar() +
                                    "' must be '0'-'9' or '" + L_ESC.toChar() +
                                    "', but found " + (if (i < l) ("symbol '" + b + "' with code " + b +
                        ...
                        lbuf.append(push_onecapture(b.code - '1'.code, soff, e))
                    }
                }
                ++i
            }
        ...
        internal fun add_value(lbuf: Buffer, soffset: Int, end: Int, repl: Any) {
            var repl = repl
            if (repl is MutableMap<*, *>) {
                repl = repl.get(push_onecapture(0, soffset, end))!!
            } else if (repl is MatchFunc) {
                repl = repl.invoke(push_captures(true, soffset, end))!!
            } else {
            ...
            } else if (repl !is String) {
                error("invalid replacement value (a " + repl.javaClass + ")")
            }
        ...
        private fun toboolean(repl: Any?): Boolean {
            if (repl == null) return false
            if (repl is Boolean) return repl
            return true
        }

        fun push_captures(wholeMatch: Boolean, soff: Int, end: Int): Array<String?>? {
            val nlevels = if (this.level == 0 && wholeMatch) 1 else this.level
            when (nlevels) {
                0 -> return null
                1 -> return arrayOf<String?>(push_onecapture(0, soff, end))
            }
            val v = arrayOfNulls<String>(nlevels)
            for (i in 0..<nlevels) v[i] = push_onecapture(i, soff, end)
            return v
        }

        private fun push_onecapture(i: Int, soff: Int, end: Int): String {
            if (i >= this.level) {
                if (i == 0) {
        ...
        fun classend(poffset: Int): Int {
            ...
            when (p.get(poffset++).code) {
                ...
                '['.code -> {
                    if (poffset != p.length && p.get(poffset) == '^') poffset++
                    do {
                        if (poffset == p.length) {
                            error("malformed pattern (missing ']')")
                        }
                        if (p.get(poffset++).code == L_ESC && poffset < p.length) poffset++ /* skip escapes (e.g. '%]') */
                    } while (poffset == p.length || p.get(poffset) != ']')
                    return poffset + 1
                }
        ...
        fun matchbracketclass(c: Int, poff: Int, ec: Int): Boolean {
            ...
            while (++poff < ec) {
                if (p.get(poff).code == L_ESC) {
                    poff++
                    if (match_class(c, p.get(poff).code)) return sig
                } else if ((p.get(poff + 1) == '-') && (poff + 2 < ec)) {
                    poff += 2
                    if (p.get(poff - 2).code <= c && c <= p.get(poff).code) return sig
                } else if (p.get(poff).code == c) return sig
            }
            return !sig
        }

        fun singlematch(c: Int, poff: Int, ep: Int): Boolean {
            when (p.get(poff).code) {
                '.'.code -> return true
                L_ESC -> return match_class(c, p.get(poff + 1).code)
                '['.code -> return matchbracketclass(c, poff, ep - 1)
                else -> return p.get(poff).code == c
            }
        }
        ...
        fun match(soffset: Int, poffset: Int): Int {
            ...
            try {
                while (true) {
                    ...
                    when (p.get(poffset).code) {
                        '('.code -> if (++poffset < p.length && p.get(poffset) == ')') return start_capture(
                            soffset,
                            poffset + 1,
                            CAP_POSITION
                        )
                        ...
                        L_ESC -> {
                            ...
                            when (p.get(poffset + 1)) {
                                ...
                                'f' -> {
                                    poffset += 2
                                    if (poffset == p.length || p.get(poffset) != '[') {
                                        error("missing '[' after '%f' in pattern")
                                    }
                                    val ep = classend(poffset)
                                    val previous =
                                        (if (soffset == 0) '\u0000' else s.get(soffset - 1)).code
                                    val next =
                                        (if (soffset == s.length) '\u0000' else s.get(soffset)).code
                    ...
                    val ep = classend(poffset)
                    val m = soffset < s.length && singlematch(s.get(soffset).code, poffset, ep)
                    val pc = (if (ep < p.length) p.get(ep) else '\u0000').code

                    when (pc) {
                        '?'.code -> {
                            var res: Int = -1
                            if (m && ((match(soffset + 1, ep + 1).also {
                                    res = it
                                }) != -1)) return res
                            poffset = ep + 1
                            continue
        ...
        fun max_expand(soff: Int, poff: Int, ep: Int): Int {
            ...
            while (soff + i < s.length &&
                singlematch(s.get(soff + i).code, poff, ep)
            ) i++
            while (i >= 0) {
        ...
        fun min_expand(soff: Int, poff: Int, ep: Int): Int {
            ...
            while (true) {
                ...
                if (res != -1) return res
                else if (soff < s.length && singlematch(s.get(soff).code, poff, ep)) soff++
                else return -1
            }
        }
        ...
        fun end_capture(soff: Int, poff: Int): Int {
            ...
            var res: Int = -1
            clen[l] = soff - cinit[l]
            if ((match(soff, poff).also { res = it }) == -1) clen[l] = CAP_UNFINISHED
            return res
        }
        ...
        fun matchbalance(soff: Int, poff: Int): Int {
            ...
            val b = p.get(poff).code
            if (s.get(soff).code != b) return -1
            val e = p.get(poff + 1).code
            var cont = 1
            while (++soff < slen) {
                if (s.get(soff).code == e) {
                    if (--cont == 0) return soff + 1
                } else if (s.get(soff).code == b) cont++
            }
            return -1
        }
        ...
        companion object {
            fun match_class(c: Int, cl: Int): Boolean {
                ...
                val cdata = if (c > 255) c else CHAR_TABLE[c].toInt()

                val res: Boolean
                when (lcl) {
                    'a' -> res = (cdata and MASK_ALPHA.toInt()) != 0
                    'd' -> res = (cdata and MASK_DIGIT.toInt()) != 0
                    'l' -> res = (cdata and MASK_LOWERCASE.toInt()) != 0
                    'u' -> res = (cdata and MASK_UPPERCASE.toInt()) != 0
                    'c' -> res = (cdata and MASK_CONTROL.toInt()) != 0
                    'p' -> res = (cdata and MASK_PUNCT.toInt()) != 0
                    's' -> res = (cdata and MASK_SPACE.toInt()) != 0
                    'g' -> res =
                        (cdata and (MASK_ALPHA.toInt() or MASK_DIGIT.toInt() or MASK_PUNCT.toInt())) != 0

                    'w' -> res = (cdata and (MASK_ALPHA.toInt() or MASK_DIGIT.toInt())) != 0
                    'x' -> res = (cdata and MASK_HEXDIGIT.toInt()) != 0
                    'z' -> res = (c == 0)
                    'h' -> res = (c >= 0x4E00 && c <= 0x9FA5) //基本汉字 20902字 4E00-9FA5
                            ...
                            || (c == 0x3007) //〇 1字 3007
                    'e' -> return Arrays.binarySearch(EMOJI_TABLE, c) > -1
                    else -> return cl == c
                }
                return (lcl.code == cl) == res
            }

            fun equals(a: CharArray, i: Int, b: CharArray, j: Int, n: Int): Boolean {
                var i = i
                var j = j
                var n = n
    ...
    class MatchResult {
        ...
        private var matchs: Array<String?>? = null
        private var start = -1
        private var end = -1
        ...
        constructor(v: Array<String?>) {
            this.matchs = v
            n = v.size
            if (n > 0) s = v[0]
        }

        constructor(start: Int, end: Int, v: Array<String?>) {
            this.start = start
            this.end = end
            this.matchs = v
        }
    ...
    internal class Buffer {
        ...
        constructor(initialCapacity: Int = DEFAULT_CAPACITY) {
            bytes = CharArray(initialCapacity)
            length = 0
            offset = 0
            value = null
        }
        ...
        fun setvalue(value: String?): Buffer {
            bytes = NOBYTES
            length = 0
            offset = length
            this.value = value
        ...
        fun append(b: Byte): Buffer {
            makeroom(0, 1)
            bytes[offset + length++] = Char(b.toUShort())
            return this
        }
        ...
        fun append(str: String): Buffer {
            ...
            val n = str.length
            makeroom(0, n)
            encodeToUtf8(c, c.size, bytes, offset + length)
            length += n
            return this
        }

        fun concatTo(lhs: Any): Buffer {
            return setvalue(value + lhs.toString())
        }

        fun concatTo(lhs: String): Buffer {
            return if (value != null) setvalue(lhs + value) else prepend(lhs)
        }

        fun concatTo(lhs: Number): Buffer {
            return if (value != null) setvalue(value + lhs.toString()) else prepend(lhs.toString())
        }

        fun prepend(s: String): Buffer {
            val n = s.length
            makeroom(n, 0)
        ...
        fun makeroom(nbefore: Int, nafter: Int) {
            if (value != null) {
                ...
                length = s!!.length
                offset = nbefore
                bytes = CharArray(nbefore + length + nafter)
        ...
        fun realloc(newSize: Int, newOffset: Int) {
            if (newSize != bytes.size) {
                val newBytes = CharArray(newSize)
        ...
        companion object {
            ...
            private val NOBYTES = charArrayOf()

            fun encodeToUtf8(chars: CharArray, nchars: Int, bytes: CharArray, off: Int): Int {
                var c: Char
                var j = off
                for (i in 0..<nchars) {
                    c = chars[i]
    ...
    interface MatchFunc {
        fun invoke(r: Array<String?>?): String?
    }
}
