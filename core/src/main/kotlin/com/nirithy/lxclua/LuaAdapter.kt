...
class LuaAdapter(
    ...
    private val mBaseData: LuaTable<Int?, LuaTable<String?, Any?>?>?
    ...
    private val mLayout: LuaTable<*, *>?
    var data: LuaTable<Int?, LuaTable<String?, Any?>?>?
        private set
    private var mTheme: LuaTable<String?, Any?>? = null

    private var mPrefix: CharSequence? = null
    ...
    private val mHandler: Handler = object : Handler() {
        override fun handleMessage(msg: Message) {
            if (msg.what == 0) {
                ...
                try {
                    val newValues = LuaTable<Int?, LuaTable<String?, Any?>?>(
                        data!!.getLuaState()
                    )
                    mLuaFilter!!.call(mBaseData, newValues, mPrefix)
                    this@LuaAdapter.data = newValues
                    notifyDataSetChanged()
    ...
    private var mLuaFilter: LuaFunction<*>? = null

    constructor(context: LuaContext, layout: LuaTable<*, *>?) : this(context, null, layout)
    ...
    init {
        ...
        L.newTable()
        loadlayout.call(mLayout, L.getLuaObject(-1), AbsListView::class.java)
        L.pop(1)
    }

    fun setAnimation(animation: LuaFunction<Animation?>?) {
    ...
    fun addAll(items: LuaTable<Int?, LuaTable<String?, Any?>?>) {
        ...
        for (i in 1..len) insert.call(mBaseData, items.get(i))
        if (mNotifyOnChange) notifyDataSetChanged()
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
        ...
        if (convertView == null) {
            try {
                ...
                L.pop(1)
                view = loadlayout.call(mLayout, holder, AbsListView::class.java)
                view!!.setTag(holder)
            } catch (e: LuaException) {
    ...
    fun setFilter(filter: LuaFunction<*>?) {
        mLuaFilter = filter
    }
    ...
    private inner class ArrayFilter : Filter() {
        override fun performFiltering(prefix: CharSequence?): FilterResults? {
            ...
            mPrefix = prefix
            if (this@LuaAdapter.data == null) return results
            if (mLuaFilter != null) {
                mHandler.sendEmptyMessage(1)
                return null
            }

            results.values = this@LuaAdapter.data
