...
private constructor() : Thread.UncaughtExceptionHandler {
    ...
    //用来存储设备信息和异常信息
    private val infos: MutableMap<String?, String?> = LinkedHashMap<String?, String?>()
    ...
    fun collectDeviceInfo(ctx: Context) {
        ...
        for (field in fields) {
            try {
                ...
                if (obj is Array<*>) infos.put(
                    field.getName(),
                    (obj as Array<Any?>).contentToString()
                )
                else infos.put(field.getName(), obj.toString())
                Log.d(TAG, field.getName() + " : " + field.get(null))
            } catch (e: Exception) {
                Log.e(TAG, "an error occured when collect crash info", e)
            }
        }
        fields = Build.VERSION::class.java.getDeclaredFields()
        for (field in fields) {
            try {
                field.setAccessible(true)
                val obj = field.get(null)
