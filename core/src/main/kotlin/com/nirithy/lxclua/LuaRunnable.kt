...
class LuaRunnable : Thread, Runnable, LuaMetaTable, LuaGcable {
    ...
    override fun run() {
        try {
            if (L == null) {
                ...
                if (mBuffer != null) newLuaRunnable(mBuffer, *mArg!!)
                else newLuaRunnable(mSrc!!, *mArg!!)
            }
        } catch (e: LuaException) {
            mLuaContext.sendMsg(e.message)
            return
        }
        if (mIsLoop) {
            Looper.prepare()
    ...
    private fun newLuaRunnable(str: String, vararg args: Any?) {
        try {
            ...
            mLuaContext.sendMsg(this.toString() + " " + e.message)
            quit()
        }
    }

    private fun newLuaRunnable(buf: ByteArray?, vararg args: Any?) {
        try {
            var ok = 0
            L!!.setTop(0)
    ...
    private fun runFunc(funcName: String?, vararg args: Any?) {
        try {
            ...
            mLuaContext.sendMsg(funcName + " " + e.message)
        }
    }
    ...
    private inner class ThreadHandler : Handler() {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                2 -> newLuaRunnable(data.getString("data")!!)
