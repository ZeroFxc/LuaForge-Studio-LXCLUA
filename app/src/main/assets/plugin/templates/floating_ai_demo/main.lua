...
local useWebUI = false   -- 切换为 true 使用 WebUI 面板
    ...
    plugin.ai.chatStream(
        ...
        function(success, content, error)
            ...
            if success and accumulated ~= "" then
                ...
                plugin.logger.error("FloatingAI", "生成失败: " .. (error or "未知错误"), error)
                if ballId then
    ...
    plugin.ai.chatStreamToEditor(
        ...
        function(success, content, error)
            ...
            if success then
                ...
                plugin.logger.error("FloatingAI", "流式→编辑器 失败: " .. (error or "未知错误"), error)
                if ballId then
                    plugin.floating.updatePanelTitle(ballId, "AI 代码生成 - 失败")
                end
                showTip("AI 生成失败: " .. (error or "未知错误"))
    ...
    plugin.ai.chatToEditor(
        ...
        function(success, content, error)
            ...
            if success then
                ...
                plugin.logger.error("FloatingAI", "一次性→编辑器 失败: " .. (error or "未知错误"), error)
                if ballId then
                    plugin.floating.updatePanelTitle(ballId, "AI 代码生成 - 失败")
                end
                showTip("AI 生成失败: " .. (error or "未知错误"))
...
local function generateCode(prompt)
    ...
    if not plugin.ai.isAvailable() then
        plugin.logger.warn("FloatingAI", "AI 服务不可用: isConfigured=" .. tostring(plugin.ai.getConfig()["enabled"]))
...
local function createFloatingBall()
    ...
    ballId = plugin.floating.createBall(
        ...
        function(id)
            -- 点击悬浮球回调
            if useWebUI then
                -- WebUI 模式：加载 web/panel.html
                local ok = plugin.floating.showPanelWebUI(id, "AI 代码生成", "panel.html")
                if not ok then
                    plugin.logger.warn("FloatingAI", "panel.html 不存在，回退到默认输入模式")
    ...
    if ballId then
        ...
        -- 示例：自定义悬浮球样式
        -- plugin.floating.updateBallColor(ballId, "#4CAF50")       -- 背景色
        -- plugin.floating.updateBallTextColor(ballId, "#FFFFFF")   -- 文字色
        -- plugin.floating.updateBallSize(ballId, 48)               -- 大小(dp)
        -- plugin.floating.updateBallText(ballId, "AI")             -- 文字

        -- 示例：控制手柄显示
        -- plugin.floating.setResizeHandleEnabled(ballId, true)     -- 显示手柄
        ...
        plugin.logger.error("FloatingAI", "悬浮球创建失败")
    end
end
...
plugin.events.register("onPluginDisabled", function(disabledPluginId)
    ...
    plugin.ai.cancelAll()  -- 取消进行中的 AI 请求
    destroyFloatingBall()
end)
